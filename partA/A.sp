...
* ---------------------
* The netlist given in the assignment uses ".ic v(q)=0 v(qb)='VDD'" together with
* capacitor IC= and "tran uic".  Those are parsed once, so the stored state and the
* bitline precharge level cannot follow VDD when it is altered inside a .control
* loop (Task 3).  Instead the cell is brought to its state by real circuitry:
*   Swr  : a 100 ohm write-assist switch holding Q at 0 until t = 0.45 ns
*   Spr* : precharge switches tying BL/BLB to the VDD rail until t = 0.95 ns
* Both open before the wordline rises at t = 1.0 ns, so the read is undisturbed and
* the whole cell tracks whatever VDD is set to.
...
* ---------------- state initialisation / precharge ----------------
.model swmod sw vt=0.5 vh=0.0 ron=100 roff=1e12
.model prmod sw vt=0.5 vh=0.0 ron=50  roff=1e12

Vwr  wrc 0 PWL(0 1 0.40n 1 0.45n 0)          $ holds Q low, then releases
Swr  q   0   wrc 0 swmod

Vpre prc 0 PWL(0 1 0.90n 1 0.95n 0)          $ precharges BL/BLB to VDD
Spr1 bl  vdd prc 0 prmod
Spr2 blb vdd prc 0 prmod
